...
local last_update_nameplate_size_call = nil
-- UpdateNameplateSize
local function UpdateNameplateSize(plate, show, cWidth, cHeight)
	...
	if last_update_nameplate_size_call == nil then
		last_update_nameplate_size_call = GetTime()
	end

	if GetTime() - last_update_nameplate_size_call > 0.1 then
		last_update_nameplate_size_call = GetTime()
...
do
	...
	function UpdateUnitCondition(plate, unitid)
		...
		local c = GetQuestDifficultyColor(unit.level)
